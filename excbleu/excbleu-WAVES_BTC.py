...
while True:
    try:
        ...
        while contador <= 1000000:
            ...
            urlGetOrderBookBLEU_BUY_WAVES_BTC = config.urlGetOrderBookBLEU_BUY_WAVES_BTC
            rBLEU = requests.get(urlGetOrderBookBLEU_BUY_WAVES_BTC)
            jBLEU = json.loads(rBLEU.text)
            quantidade_comprandoBLEU = jBLEU['result']['buy'][0]['Quantity']

            urlGetOrderBookBLEU_SELL_WAVES_BTC = config.urlGetOrderBookBLEU_SELL_WAVES_BTC
            rBLEU = requests.get(urlGetOrderBookBLEU_SELL_WAVES_BTC)
            jBLEU = json.loads(rBLEU.text)
            quantidade_vendendoBLEU = jBLEU['result']['sell'][0]['Quantity']

            urlGetOrderBookEXC_BUY_WAVES_BTC = config.urlGetOrderBookEXC_BUY_WAVES_BTC
            rEXC = requests.get(urlGetOrderBookEXC_BUY_WAVES_BTC)
            jEXC = json.loads(rEXC.text)
            quantidade_comprandoEXC = jEXC['result']['buy'][0]['Quantity']

            urlGetOrderBookEXC_SELL_WAVES_BTC = config.urlGetOrderBookEXC_SELL_WAVES_BTC
            rEXC = requests.get(urlGetOrderBookEXC_SELL_WAVES_BTC)
            jEXC = json.loads(rEXC.text)
            quantidade_vendendoEXC = jEXC['result']['sell'][0]['Quantity']

            # print('-' * 10)

            # print('\nMERCADO EXC: WAVES_BTC')
            urlGettickerEXC = config.urlGettickerEXC_WAVES_BTC
            ...
            # print('\nMERCADO BLEU: WAVES_BTC')
            urlGettickerBLEU = config.urlGettickerBLEU_WAVES_BTC
...
####################################################################################
            ...
            if vendaAtivoBLEU < compraAtivoEXC and compraAtivoEXC - vendaAtivoBLEU > diferencaSatoshi_WAVES_BTC:
                ########################
                #     OPORTUNIDADE     #
                ########################
                print(f'\n>> OPORTUNIDADE << VENDA na BLEU mais barata que COMPRA na EXC e diferença acima de: {diferencaSatoshi_WAVES_BTC:.8f} satoshi\n')
...
##########################################################################################
                    ...
                    faz_compraBLEU = key_secretBLEU.buy_limit('WAVES_BTC', vendaAtivoBLEU, quantidade_vendendoBLEU)
                    ...
                    if respostaFazCompra == 'ERR_INSUFICIENT_BALANCE':
                        ...
                        faz_compra = key_secretBLEU.buy_limit('WAVES_BTC', vendaAtivoBLEU, minimaCompra_WAVES_BTC)
                        print(f'Comprando... {minimaCompra_WAVES_BTC:.8f} WAVES')
                        print(faz_compra)

                        # Mensagem Telegram
                        config.mensagem_telegram('>>> FAZENDO COMPRA MÍNIMA <<<')
                        config.mensagem_telegram(f'>> VALORES: \nCompra: {vendaAtivoBLEU:.8f} \nVenda: {compraAtivoEXC:.8f} \nQuantidade: {minimaCompra_WAVES_BTC:.8f}\n')
                        config.mensagem_telegram(f'>> AÇÃO:\nComprando na BLEU... {minimaCompra_WAVES_BTC:.8f}\n{faz_compra}\n')
                        ...
                        ######################################
                        print(f'\nTransferindo... {minimaCompra_WAVES_BTC:.8f} WAVES')
                        mCOMPRA = str(minimaCompra_WAVES_BTC)
                        urlDirecttransferBLEU = 'https://bleutrade.com/api/v3/private/directtransfer?apikey=' + keyBLEU + '&nonce=' + str(int(time.time())) + '&asset=WAVES&quantity=' + mCOMPRA + '&exchangeto=2&' + 'accountto=' + emailEXC
                        # sign
                        ...
                        ######################################################
                        print(f'\nVENDENDO... {minimaCompra_WAVES_BTC:.8f} WAVES')
                        faz_vendaEXC = key_secretEXC.sell_limit('WAVES_BTC', compraAtivoEXC, minimaCompra_WAVES_BTC)
                        print(faz_vendaEXC)
                        ...
                        ##############################################################
                        print('\nVendendo na EXC...')
                        faz_vendaEXC = key_secretEXC.sell_limit('WAVES_BTC', compraAtivoEXC, quantidade_vendendoBLEU)
                        print(faz_vendaEXC)
...
##########################################################################################
                    ...
                    faz_compraBLEU = key_secretBLEU.buy_limit('WAVES_BTC', vendaAtivoBLEU, quantidade_comprandoEXC)
                    ...
                    if respostaFazCompra == 'ERR_INSUFICIENT_BALANCE':
                        ...
                        # Faz TRANSFERÊNCIA da BLEU para EXC #
                        ######################################
                        print(f'Transferindo... {minimaCompra_WAVES_BTC:.8f} WAVES')
                        mCOMPRA = str(minimaCompra_WAVES_BTC)
                        urlDirecttransferBLEU = 'https://bleutrade.com/api/v3/private/directtransfer?apikey=' + keyBLEU + '&nonce=' + str(
                            int(
                                time.time())) + '&asset=WAVES&quantity=' + mCOMPRA + '&exchangeto=2&' + 'accountto=' + emailEXC
                        # sign
                        sign = hmac.new(secretBLEU.encode(), urlDirecttransferBLEU.encode(), hashlib.sha512).hexdigest()
                        # Adding sign to header
                        headers = {'apisign': sign}
                        ...
                        # Faz VENDA na EXC: MOEDA_MERCADO, preço, quantidade #
                        ######################################################
                        print(f'Vendendo... {minimaCompra_WAVES_BTC:.8f} WAVES')
                        faz_vendaEXC = key_secretEXC.sell_limit('WAVES_BTC', compraAtivoEXC, minimaCompra_WAVES_BTC)
                        print(faz_vendaEXC)
                        ...
                        ##############################################################
                        print('\nVendendo na EXC...')
                        faz_vendaEXC = key_secretEXC.sell_limit('WAVES_BTC', compraAtivoEXC, quantidade_comprandoEXC)
                        print(faz_vendaEXC)
...
####################################################################################
            ...
            elif vendaAtivoBLEU < compraAtivoEXC and compraAtivoEXC - vendaAtivoBLEU < diferencaSatoshi_WAVES_BTC:
                print(f'\n>> AGUARDE << VENDA na BLEU mais barata que COMPRA na EXC, mas diferença abaixo de: {diferencaSatoshi_WAVES_BTC:.8f} satoshi\n')
...
####################################################################################
            ...
            if vendaAtivoEXC < compraAtivoBLEU and compraAtivoBLEU - vendaAtivoEXC > diferencaSatoshi_WAVES_BTC:
                ########################
                #     OPORTUNIDADE     #
                ########################
                print(f'\n>> OPORTUNIDADE << VENDA na EXC mais barata que COMPRA na BLEU e diferença acima de: {diferencaSatoshi_WAVES_BTC:.8f} satoshi\n')
...
####################################################################################
                    faz_compraEXC = key_secretEXC.buy_limit('WAVES_BTC', vendaAtivoBLEU, quantidade_vendendoEXC)
                    # print(faz_compra['result'])
                    print(faz_compraEXC)
                    ...
                    if respostaFazCompraEXC == 'ERR_INSUFICIENT_BALANCE':
                        ...
                        ################################################################
                        print(' >>> FAZENDO COMPRA MÍNIMA <<<\n')
                        faz_compra = key_secretEXC.buy_limit('WAVES_BTC', vendaAtivoEXC, minimaCompra_WAVES_BTC)
                        print(f'COMPRANDO... {minimaCompra_WAVES_BTC:.8f} WAVES')
                        print(faz_compra)
                        ...
                        config.mensagem_telegram('>>> FAZENDO COMPRA MÍNIMA <<<')
                        config.mensagem_telegram(f'>> VALORES: \nCompra: {vendaAtivoEXC:.8f} \nVenda: {compraAtivoBLEU:.8f} \nQuantidade: {minimaCompra_WAVES_BTC:.8f}\n')
                        config.mensagem_telegram(f'>> AÇÃO:\nComprando na EXC... {minimaCompra_WAVES_BTC:.8f}\n{faz_compra}\n')
                        ...
                        urlDirecttransferEXC = 'https://trade.exccripto.com/api/v3/private/directtransfer?apikey=' + keyEXC + '&nonce=' + str(
                            int(
                                time.time())) + '&asset=WAVES&quantity=' + mCOMPRA + '&exchangeto=1&' + 'accountto=' + emailBLEU
                        ...
                        ######################################################
                        print(f'VENDENDO... {minimaCompra_WAVES_BTC:.8f} WAVES')
                        faz_vendaBLEU = key_secretBLEU.sell_limit('WAVES_BTC', compraAtivoBLEU, minimaCompra_WAVES_BTC)
                        print(faz_vendaBLEU)
                        ...
                        urlDirecttransferEXC = 'https://trade.exccripto.com/api/v3/private/directtransfer?apikey=' + keyEXC + '&nonce=' + str(
                            int(
                                time.time())) + '&asset=WAVES&quantity=' + mCOMPRA2 + '&exchangeto=1&' + 'accountto=' + emailBLEU
                        ...
                        faz_vendaBLEU = key_secretBLEU.sell_limit('WAVES_BTC', compraAtivoEXC, quantidade_vendendoEXC)
                        print(faz_vendaBLEU)
...
####################################################################################
                    ...
                    faz_compraEXC = key_secretEXC.buy_limit('WAVES_BTC', vendaAtivoBLEU, quantidade_comprandoBLEU)
                    # print(faz_compra['result'])
                    print(faz_compraEXC)
                    respostaResult = faz_compraEXC['result']
                    ...
                    if respostaFazCompraEXC == 'ERR_INSUFICIENT_BALANCE':
                        ...
                        faz_vendaBLEU = key_secretBLEU.sell_limit('WAVES_BTC', compraAtivoEXC, quantidade_comprandoBLEU)
                        print(faz_vendaBLEU)
...
####################################################################################
            ...
            elif vendaAtivoEXC < compraAtivoBLEU and compraAtivoBLEU - vendaAtivoEXC < diferencaSatoshi_WAVES_BTC:
                print(f'\n>> AGUARDE << Venda na EXC está mais barata que compra na BLEU, mas diferença abaixo de {diferencaSatoshi_WAVES_BTC:.8f} satoshi\n')
