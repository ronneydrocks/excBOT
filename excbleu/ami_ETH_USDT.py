...
#####################################
# SALDO INICIAL EM ETH
saldoInicial_EXC = config.saldoInicial_EXC
saldoInicial_BLEU = config.saldoInicial_BLEU
#####################################
# VALOR ORDEM MÍNIMA
minimaCompra_ETH_USDT = config.minimaCompra_ETH_USDT
#####################################
# print('\nMERCADO EXC: ETH_USDT')
...
if acaoCV == 'COMPRA':
    ...
    print('>> VALOR FINAL: Até qual preço deseja comprar (Ex: 150.0)')
    ...
    while c < nATIVOS:
        ...
        print(f'{balanceATIVO:.8f} {ATIVO} = {eATIVO:.8f} ETH')

        # SOMA SALDO EQUIVALENTE EM ETH
        somaEqETH_EXC += eATIVO
        c += 1

    ######################################################################

    print(f'\nSaldo inicial na EXC: {saldoInicial_EXC:.8f} ETH')
    print(f'Saldo atual na EXC: {somaEqETH_EXC:.8f} ETH')

    # TELEGRAM
    # config.mensagem_telegram(f'
    # Saldo inicial na EXC: {saldoInicial_EXC:.8f} ETH \nSaldo atual na EXC: {somaEqETH_EXC:.8f} ETH')

    # LOG
    log = open('logs/saldo.txt', 'a')
    log.write(f'Saldo inicial na EXC: {saldoInicial_EXC:.8f} ETH \nSaldo atual na EXC: {somaEqETH_EXC:.8f} ETH\n')
    log.write('-' * 10)
    log.write('\n')
    log.close()
    ...
    if porcentagemUso == 100:
        ...
        # ORDEM MÍNIMA
        q1 = saldoDisponivelATIVO / minimaCompra_ETH_USDT
        q1 = int(q1)
        print(f'Valor ordem mínima: {minimaCompra_ETH_USDT:.8f}')
        ...
        quantidadeOrdem = minimaCompra_ETH_USDT
        ...
        while rateOrdem > rangeFinal:
            ...
            criaOrdem = key_secretEXC.buy_limitami('ETH_USDT', rateAMI, rateOrdem, quantidadeOrdem)
    ...
    print('\n>> VENDA\n')
    print('ATENÇÃO: Digite apenas números!!!\n')
    print('>> VALOR FINAL: Até qual preço deseja vender (Ex: 350)')
    rangeFinal = input('Digite aqui: ')
    rangeFinal = float(rangeFinal)
    ...
    # TELEGRAM
    # config.mensagem_telegram(f'Saldo inicial na EXC: {saldoInicial_EXC:.8f} ETH \nSaldo atual na EXC: {somaEqETH_EXC:.8f} ETH')
    ...
    if porcentagemUso == 100:
        ...
        q1 = saldoDisponivelATIVO / minimaCompra_ETH_USDT
        q1 = int(q1)
        print(f'Valor ordem mínima: {minimaCompra_ETH_USDT}')
        print(f'Quantidade de ordens Mínimas: {q1}')
        ...
        while rateOrdem < rangeFinal:
            ...
            criaOrdem = key_secretEXC.sell_limitami('ETH_USDT', rateAMI, rateOrdem, quantidadeOrdem)
            print(f'Resposta: {criaOrdem}\n')
