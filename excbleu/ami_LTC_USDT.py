...
if acaoCV == 'COMPRA':
    ...
    print('>> VALOR FINAL: Até qual preço deseja comprar (Ex: 50.0)')
    ...
    while c < nATIVOS:
        ...
        print(f'{balanceATIVO:.8f} {ATIVO} = {eATIVO:.8f} LTC')

        # SOMA SALDO EQUIVALENTE EM LTC
        somaEqLTC_EXC += eATIVO
        c += 1

    ######################################################################

    print(f'\nSaldo inicial na EXC: {saldoInicial_EXC:.8f} LTC')
    print(f'Saldo atual na EXC: {somaEqLTC_EXC:.8f} LTC')

    # TELEGRAM
    # config.mensagem_telegram(f'
    # Saldo inicial na EXC: {saldoInicial_EXC:.8f} LTC \nSaldo atual na EXC: {somaEqLTC_EXC:.8f} LTC')

    # LOG
    log = open('logs/saldo.txt', 'a')
    log.write(f'Saldo inicial na EXC: {saldoInicial_EXC:.8f} LTC \nSaldo atual na EXC: {somaEqLTC_EXC:.8f} LTC\n')
    log.write('-' * 10)
    log.write('\n')
    log.close()
    ...
    if porcentagemUso == 100:
        ...
        # ORDEM MÍNIMA
        q1 = saldoDisponivelATIVO / minimaCompra_LTC_USDT
        q1 = int(q1)
        print(f'Valor ordem mínima: {minimaCompra_LTC_USDT:.8f}')
        ...
        quantidadeOrdem = minimaCompra_LTC_USDT
        ...
        while rateOrdem > rangeFinal:
            ...
            criaOrdem = key_secretEXC.buy_limitami('LTC_USDT', rateAMI, rateOrdem, quantidadeOrdem)
    ...
    print('\n>> VENDA\n')
    print('ATENÇÃO: Digite apenas números!!!\n')
    print('>> VALOR FINAL: Até qual preço deseja vender (Ex: 500)')
    rangeFinal = input('Digite aqui: ')
    rangeFinal = float(rangeFinal)
    ...
    # TELEGRAM
    # config.mensagem_telegram(f'Saldo inicial na EXC: {saldoInicial_EXC:.8f} LTC \nSaldo atual na EXC: {somaEqLTC_EXC:.8f} LTC')
    ...
    if porcentagemUso == 100:
        ...
        q1 = saldoDisponivelATIVO / minimaCompra_LTC_USDT
        q1 = int(q1)
        print(f'Valor ordem mínima: {minimaCompra_LTC_USDT}')
        print(f'Quantidade de ordens Mínimas: {q1}')
        ...
        while rateOrdem < rangeFinal:
            ...
            criaOrdem = key_secretEXC.sell_limitami('LTC_USDT', rateAMI, rateOrdem, quantidadeOrdem)
            print(f'Resposta: {criaOrdem}\n')
